...
#define	IF_ENDIAN	1	//big
#elif (defined __BYTE_ORDER) && __BYTE_ORDER == __LITTLE_ENDIAN
#define	IF_ENDIAN	0	//little
#else
#error ipf_linux.h	-	endian type not defined!
#endif

#define	IF_PLF_OS	2		//linux
...
///////////////// arm
#if (defined __aarch64__) || (defined __arm__) || (defined IF_ARCH_ARM)
#define Linux_exe_arch "arm"
#define IF_PLF_CPU		(IF_ENDIAN+2)
#define INTERLOCK_ALIGN_CHECK 1

#if !(defined IF_PTRSIZE) && (defined __arm__)
#define IF_PTRSIZE 4
#endif
#if !(defined IF_PTRSIZE) && (defined __aarch64__)
#define IF_PTRSIZE 8
#endif
...
///////////////// intel
#if (defined __i386__) || (defined __x86_64__) || (defined IF_ARCH_INTEL)
#define Linux_exe_arch "i"
#define IF_PLF_CPU		1

#if !(defined IF_PTRSIZE) && (defined __i386__)
#define IF_PTRSIZE 4
#endif
#if !(defined IF_PTRSIZE) && (defined __x86_64__)
#define IF_PTRSIZE 8
#endif
...
#ifndef IF_PTRSIZE
#error ipf_linux.h	-	pointer size not defined!
#endif

#ifndef Linux_exe_arch
#error	ipf_linux.h	-	architecture not defined!
#else
#ifndef IF_PC_EXE_NAME
#define IF_PC_EXE_NAME		Linux_exe_pfx Linux_exe_arch Linux_exe_bits
#endif
#endif
...
#define PP_STR_INT(x) PP_STR_HELPER (x)
#pragma message("Pointer size = " PP_STR_INT(IF_PTRSIZE))
#pragma message("Endianity = "  PP_STR_INT(IF_ENDIAN))
#pragma message("Architecture = " Linux_exe_arch)
*/


#define IF_LINUX	(IF_PTRSIZE*8)
...
#define	pR_ProcessId											(DWORD)getpid
...
#define pR_CreateMapping(wr,size,name,hnd,offs)					Linux_CreateMapping(wr,size,name,hnd,offs)
#define pR_FlushMapping(p)										Linux_FlushMapping(p)
#define pR_DeleteMapping(p,size)								Linux_DeleteMapping(p,size)

#define pR_MapGenericMask										Ipos_MapGenericMask

#define pR_CreateDirNotificationEvent(name,b_sub,b_full)		Linux_LnotStart(name)
#define	pR_CloseDirNotificationEvent							Linux_LnotStop
#define	pR_DirNotificationNext(hEvt)

#define	pR_GetComputerName										Linux_GetComputerName
...
#define pR_GetACP												Linux_GetCodePage

#define	pR_PreInitOSSpecific									LinuxPreInit
#define	pR_InitOSSpecific										LinuxInit
#define	pR_ReinitOSSpecific										LinuxReinit
...
#define	pR_SetSvcOpt(opt,val)									Linux_SetSvcOpt( opt,val)
#define	pR_GetMainResource										Linux_GetMainResource

#define	pR_osu_enum_users										Linux_osu_enum_users
#define	pR_osu_logon_user										Linux_osu_logon_user
#define	pR_NormalizeUserName									Linux_NormalizeUserName
...
#define os_uxt_dls_bias											Ipos_uxt_get_dls_bias
...
#define pR_kbhit												Linux_kbhit
#define pR_getch												Linux_getch
#define pR_getche												Linux_getche
...
#define	pR_GetFullUserName(UserName,FullName,cbFullName)		Linux_GetFullUserName(UserName,FullName,cbFullName)
...
BOOL			LinuxPreInit();
VOID			LinuxReinit();
int				Linux_GetCodePage();
LPSTR			Linux_GetExeName();
LPSTR			Linux_BuildDate();
DWORD			Linux_BuildDateUT();
BOOL			Linux_InstallService(LPSTR svc_name, LPSTR svc_descr,LPSTR path,LPSTR base_path);
BOOL			Linux_RemoveService(LPSTR svc_name, LPSTR path);
BOOL			Linux_CommandService(LPSTR svc_name,INT	cmd);
BOOL			Linux_SvcScript(LPSTR svc_name, LPSTR svc_descr,LPSTR path,LPSTR base_path);

PVOID	Linux_CreateMapping(BOOL	wr,	DWORD	size,LPSTR	name,HANDLE	hnd,DWORD offs);
VOID	Linux_FlushMapping(PVOID	p);
VOID	Linux_DeleteMapping(PVOID	p,DWORD size);
...
VOID	Linux_SetSvcOpt(LPSTR opt,LPSTR value);
VOID	Linux_FnMB2UTF(LPSTR text, LPSTR buf, DWORD cb_buf);
VOID	Linux_FnUTF2MB(LPSTR text,LPSTR buf, DWORD cb_buf);
...
#define	cfsSrvInstallService	Linux_InstallService
...
#define FN_MB2UTF(x)	if(cfsNonAnsi(x) || cfsDivWindows(x)) { \
		...
		Linux_FnMB2UTF(x,  _mb2u, _mb2u_len);\
		x=_mb2u; \
	}

#define FN_UTF2MB(x)	if(cfsNonAnsi(x))Linux_FnUTF2MB(x, x , pR_strlen(x)+1);


LPSTR	Linux_osu_enum_users(BOOL b_comment);
BOOL	Linux_osu_logon_user(LPSTR user, LPSTR pw, PBOOL pAdmin);
VOID	Linux_NormalizeUserName(LPSTR uname,LPSTR osun,DWORD cb_osun);
VOID	Linux_GetFullUserName(LPSTR UserName,LPSTR FullName,DWORD cbFullName);
LPSTR	Linux_GetAltBaseRoot();


PBYTE	Linux_GetMainResource(PDWORD pdw);

u64	Linux_GetHeapUsage();

int Linux_kbhit();
int Linux_getch();
int Linux_getche();

#define	pR_AltBaseRoot		((getuid()==0)?(Linux_GetAltBaseRoot()):NULL)
#ifdef __cplusplus
}
